...
- Denis calisthenics: for skills worked this week, check exit criteria (per-level targets in shared/calisthenics_ladders.md; level names in data.json skills.ladders). If met with clean form → advance the SKILL_STATE block in calisthenics_status.md (the only place levels are recorded) and note the next drill.
